(function () {
  ...
  function buildInquiryModal() {
    ...
    modal.innerHTML = [
      ...
      '<div class="inquiry-modal__intro card">',
      '<p class="lead">아래 내용을 입력하면 카카오톡에 붙여넣을 문의 문구를 만들어드립니다.</p>',
      '<p>아직 정해지지 않은 항목은 비워두거나 "상담 필요"로 선택해주셔도 됩니다.</p>',
      '</div>',
      ...
      '<button class="btn btn-secondary" type="button" data-inquiry-copy>문의 문구 복사하기</button>',
      '<a class="btn btn-primary" href="' + escapeHtml(KAKAO_CHANNEL_URL) + '" target="_blank" rel="noopener" data-inquiry-kakao>카카오톡 문의하기</a>',
      '<button class="btn btn-secondary" type="button" data-inquiry-edit>다시 수정하기</button>',
      '</div>',
      '</section>',
      '</div>',
      '</div>'
